...
public class InGameManager : SceneSingleton<InGameManager>
{
    ...
    private void Start()
    {
        ...
        _nextTrain.transform.position += new Vector3((_nextTrain._floor.transform.localScale.x / 1.5f)
            + (_currentTrain._floor.transform.localScale.x / 1.5f), 0,
            (_nextTrain._floor.transform.localScale.z / 3)
            + (_currentTrain._floor.transform.localScale.z / 3)) + _trainInterval;
    ...
    public void StartNextTrain(GameObject train)
    {
        ...
        Vector3 nextPosition = new Vector3((_nextTrain._floor.transform.localScale.x / 1.5f)
            + (_currentTrain._floor.transform.localScale.x / 1.5f), 0,
            (_nextTrain._floor.transform.localScale.z / 3)
            + (_currentTrain._floor.transform.localScale.z / 3)) + _trainInterval;
        _nextTrain.transform.position += nextPosition;

        _backgroundGroup.position += new Vector3(nextPosition.x, 0, nextPosition.z);
    }
